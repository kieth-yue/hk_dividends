...
def is_excluded_stock(code, name):
    if code.startswith("8"):
        return True
    if len(code) == 5 and code[0] == "0" and code[1] == "9":
        return True
    ...
    if df.empty:
        ...
        for _, row in df.iterrows():
            ...
            item_md = (
                ...
                f"💸 每手入場費：HK$ {row['lot_cost']:,.0f}（{row['lot_size']:,}股 × ${row['last_price']:.2f}）\n"
                f"📈 本次收益率：<font color='green'>**{row['yield_pct']:.2f}%**</font>　|　年度週息率：{row['annual_yield_pct']:.2f}%\n"
...
def main():
    ...
    print(f"港交所披露易返回 {len(div_records)} 條現金派息記錄（已過濾ETF/衍生品）")
    ...
    for item in div_records:
        ...
        lot_cost = last_price * lot_size  # 每手入場成本
    ...
    if not df.empty:
        ...
        print("\n===== 最終篩選結果 =====")
        print(df[["code", "name", "dividend_per_share", "yield_pct", "dividend_per_lot", "lot_cost"]].to_string(index=False))
